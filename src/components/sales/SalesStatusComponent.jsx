...
const SalesStatusComponent = ({ user, onNavigate, onLogout }) => {
    ...
    const fetchUsecases = async () => {
        try {
            ...
            const formattedUsecases = Array.isArray(response.data)
                ? response.data.map(u => ({
                    id: u.pocId,                 // ✅ integer
                    pocId: u.pocId,
                    pocName: u.pocName,
                    clientName: u.clientName,
                    name: `${u.clientName} - ${u.pocName}`, // for table / payload
                    rawData: u
                }))
                : [];
    ...
    const filteredUsecases = usecases.filter(usecase =>
        String(usecase.pocId).includes(searchUsecase) ||
        usecase.pocName?.toLowerCase().includes(searchUsecase.toLowerCase()) ||
        usecase.clientName?.toLowerCase().includes(searchUsecase.toLowerCase())
    );
                                            ...
                                            <FormControl fullWidth size={isMobile ? "small" : "medium"}>
                                                ...
                                                >
                                                    ...
                                                    {filteredUsecases.map((usecase) => {
                                                        const isInternal = false; // since pocId is numeric

                                                        { `POC-${usecase.pocId} (${usecase.clientName} - ${usecase.pocName})` }
                                                        return (
                                                            <MenuItem key={usecase.id} value={usecase.id}>
                                                                <Box sx={{
                                                                    ...
                                                                    >
                                                                        {`ID-${usecase.pocId} (${usecase.clientName} - ${usecase.pocName})`}
                                                                    </Typography>
                                    ...
                                    <Box sx={{
                                        ...
                                        {userPermissions.all_status_access && (
                                            <>
                                                ...
                                                {isCustomDate && (
                                                    <Button onClick={() => safeAction(() => handleTodayClick())} size="small" sx={{ ml: 1 }}>
                                                        Today
                                                    </Button>
                                                )}
                                            </>
                                                                    ...
                                                                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                                                                        ...
                                                                        {(userPermissions.all_status_access || status.employeeId === user?.emp_id) && (
                                                                            <>
                                                                                <IconButton
                                                                                    size="small"
                                                                                    onClick={() => safeAction(() => handleEdit(status))}
                                                                                    sx={{
                                                                                ...
                                                                                <IconButton
                                                                                    size="small"
                                                                                    onClick={() => safeAction(() => handleDeleteClick(status))}
                                                                                    sx={{
                                                                                        bgcolor: 'error.light',
