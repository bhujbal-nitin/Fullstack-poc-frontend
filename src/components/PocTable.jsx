...
const PocTable = ({ onNavigate, onLogout, user }) => {
    ...
    // Add these state variables for auto-logout
    ...
    const handleClose = () => {
        setOpen(false);
        // Validate token when modal closes
        validateToken();
    };
    ...
    const exportToExcel = (data, filename) => {
        ...
        // Set column widths
        const colWidths = columnsToExport.map(() => ({ wch: 20 }));
        ws['!cols'] = colWidths;
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Usecase Reports');

        // Generate filename with timestamp
        const date = new Date().toISOString().split('T')[0];
        const timestamp = new Date().toTimeString().split(' ')[0].replace(/:/g, '-');
        const excelFilename = filename || `${date}_${timestamp}_${data.length}_records.xlsx`;

        // Save file
        XLSX.writeFile(wb, excelFilename);
    };

    // Update the export handlers to use Excel instead of CSV
    const handleExportSelected = () => {
        ...
        const date = new Date().toISOString().split('T')[0];
        exportToExcel(selectedRows, `selected-usecases-${date}.xlsx`);
        setExportMenuAnchor(null);
        showSnackbar(`Exported ${selectedRows.length} rows to Excel with all columns`, 'success');
    };

    const handleExportAll = () => {
        const date = new Date().toISOString().split('T')[0];
        exportToExcel(filteredData, `all-usecases-${date}.xlsx`);
        setExportMenuAnchor(null);
        showSnackbar(`Exported all ${filteredData.length} rows to Excel with all columns`, 'success');
    };

    const handleExportPage = () => {
        const date = new Date().toISOString().split('T')[0];
        exportToExcel(paginatedData, `page-usecases-${date}.xlsx`);
        setExportMenuAnchor(null);
        showSnackbar(`Exported page ${page + 1} rows to Excel with all columns`, 'success');
    };
    ...
    // Fetch POC data with token validation
    const fetchPocData = async () => {
        try {
            ...
            const response = await axios.get(`${import.meta.env.VITE_API}/poc/all`, {
                headers: {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            const token = localStorage.getItem('authToken');
            await axios.delete(`${import.meta.env.VITE_API}/poc/delete/${pocToDelete.pocId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    const handleStatusChange = async (poc, newStatus) => {
        try {
            const token = localStorage.getItem('authToken');
            await axios.put(`${import.meta.env.VITE_API}/poc/updateStatus/${poc.pocId}`,
                { status: newStatus },
                {
                    headers: {
                        'Authorization': `Bearer ${token}`,
    ...
    const handleRemarkUpdate = async (poc, newRemark) => {
        try {
            const token = localStorage.getItem('authToken');
            await axios.put(`${import.meta.env.VITE_API}/poc/updateRemark/${poc.pocId}`,
                { remark: newRemark },
                {
                    headers: {
                        'Authorization': `Bearer ${token}`,
